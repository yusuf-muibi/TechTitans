...
def signup():
    try:
        ...
        if cus_signup.lower() == "signup":
            ...
            while True:
                cus_pword = input(
                    "Create your password, password must contain upper, lower and special characters and >= 8: "
                )
                if (
                    not len(cus_pword) >= 8
                    or not any(char.isupper() for char in cus_pword)
                    or not any(char.islower() for char in cus_pword)
                ):
                    print(
                        "password must contain characters greater than 8 and must include upper,lower and special characters"
                    )
                    print("pls try again")
                    continue
